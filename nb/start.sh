...
if [ $(id -u) == 0 ] ; then
    ...
    # Only attempt to change the ewsuser username if it exists
    if id ewsuser &> /dev/null ; then
        echo "Set username to: $NB_USER"
        usermod -d /home/$NB_USER -l $NB_USER ewsuser
    fi
    ...
    if [[ "$NB_USER" != "ewsuser" ]]; then
        # changing username, make sure homedir exists
        # (it could be mounted, and we shouldn't create it if it already exists)
        if [[ ! -e "/home/$NB_USER" ]]; then
            echo "Relocating home dir to /home/$NB_USER"
            mv /home/ewsuser "/home/$NB_USER"
        fi
        # if workdir is in /home/ewsuser, cd to /home/$NB_USER
        if [[ "$PWD/" == "/home/ewsuser/"* ]]; then
    ...
    if [[ "$NB_UID" == "$(id -u ewsuser)" && "$NB_GID" == "$(id -g ewsuser)" ]]; then
        ...
        if [[ "$STATUS" != "0" ]]; then
            if [[ -w /etc/passwd ]]; then
                ...
                cat /etc/passwd | sed -e "s/^ewsuser:/nayvoj:/" > /tmp/passwd
                echo "ewsuser:x:$(id -u):$(id -g):,,,:/home/ewsuser:/bin/bash" >> /tmp/passwd
